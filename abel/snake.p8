...
function update_game()
    ...
    -- put directions into state
    poke(0x5310,0) -- reset gameover
    poke(0x5300,lastdir.x)
    poke(0x5301,lastdir.y)
	...
	for f in all(food) do
		...
		and f.y==h.y%grid_height then
			game.score+=f.val*5
			game.stats[f.n]+=1
      -- update state fruit eat and send msg
      poke(0x530b,1)
      poke(0x530c,game.score)
...
function drwsnek()
  ...
  local hy=0
    ...
    --
      ...
      hx=x
      hy=y
      poke(0x530a,0) -- reset gameover
      poke(0x530b,0) -- reset eat flag
      poke(0x5302,x)
      poke(0x5303,y)
      ...
      -- set state 
      if by==hy then
        if hx==bx-12 or (bx<=12 and 112-hx==12-bx) then
          hr=1 
        end
        if hx==bx+12 or (hx<=12 and 112-bx+hx==12) then
          hl=1 
        end
      end 
      if bx==hx then 
        if hy==by-12 or (hy==112 and by-24<8) then
          hd=1
        end 
        if hy==by+12 or (hy==24 and 112-by<8) then
          hu=1
        end 
      end 
      poke(0x4304,hl)
